...
//     int read_threads_count;
//     char can_read, can_write;
// };
...
int rwlock_rlock(rwlock* lock);
int rwlock_wlock(rwlock* lock);
...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      if (which_op < search_percent) {
         rwlock_rlock(&mrwlock);
         Member(val);
         rwlock_unlock(&mrwlock);
         my_member_count++;
      } else if (which_op < search_percent + insert_percent) {
         rwlock_wlock(&mrwlock);
         Insert(val);
         rwlock_unlock(&mrwlock);
         my_insert_count++;
      } else { /* delete */
...
//     if (!lock->can_read && !lock->can_write)
//     {
//         lock->can_write = 1;
//         lock->can_read = 1;
//         lock->write_mutex.unlock();
